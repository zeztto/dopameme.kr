...
BEGIN
  ...
  LOOP
    ...
    -- 각 사용자가 참여할 예측 개수 랜덤 설정 (50~70개)
    v_target_predictions := 50 + floor(random() * 21)::INTEGER;
      ...
      IF v_option.id IS NOT NULL THEN
        ...
        IF v_test_user.dpmm_balance >= v_bet_amount THEN
          ...
          INSERT INTO predictions (
            ...
            v_option.id,
            v_bet_amount,
            NOW() - (random() * interval '30 days'), -- 최근 30일 이내 랜덤 시간
            0 -- 아직 해결되지 않음
          );
